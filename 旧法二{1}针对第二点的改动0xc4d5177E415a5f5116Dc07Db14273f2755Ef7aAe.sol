...
contract ERC721WithLicense {
    ...
    function grantLicense(
        ...
        if (exclusive) {
            for (uint i = 1; i <= licenseCount; i++) {
                if (licenses[i].tokenId == tokenId && !licenses[i].revoked && licenses[i].exclusive) {
    ...
    function revokeLicense(uint256 tokenId, uint256 licenseId) external {
        require(msg.sender == ownerOf(tokenId), "Not owner");
        require(licenses[licenseId].licensor != address(0), "License not exist");
        require(!licenses[licenseId].revoked, "Already revoked");
        licenses[licenseId].revoked = true;
        emit LicenseRevoked(tokenId, licenseId);
    }

    function getLicenseCount(uint256) external view returns (uint256) {
        return licenseCount;
    }
